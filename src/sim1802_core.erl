...
-record(core,
        ...
        , idle :: boolean() % idling until interrupt
        }).
...
-define(OP_RSXD, 16#A0). % 68A0: REGISTER STORE VIA X AND DECREMENT
...
-define(SIGNAL_INTERRUPT, interrupt).
...
-spec set_interrupt() -> ok.
set_interrupt() ->
  set_signal(?SIGNAL_INTERRUPT).
...
do_step(Core) ->
  ...
  case pred_IE_NZ(Core1) andalso is_signal(?SIGNAL_INTERRUPT) of
